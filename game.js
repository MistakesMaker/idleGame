...
function recalculateStats() { playerStats.baseClickDamage = 1; playerStats.baseDps = 0; const allItems = [...gameState.legacyItems, ...Object.values(gameState.equipment)]; for(const item of allItems) { if(item) addStatsFromItem(item); } const clickUpgradeBonus = gameState.upgrades.clickDamage * 0.5; const dpsUpgradeBonus = gameState.upgrades.dps * 1; playerStats.totalClickDamage = playerStats.baseClickDamage + clickUpgradeBonus; playerStats.totalDps = playerStats.baseDps + dpsUpgradeBonus; if (socket && socket.connected) { socket.emit('updatePlayerStats', { dps: playerStats.totalDps }); } }
...
// --- AUTOSAVE SYSTEM & RESET ---
// ... (correct)
let saveTimeout; function autoSave() { localStorage.setItem('idleRPGSaveData', JSON.stringify(gameState)); saveIndicatorEl.classList.add('visible'); if (saveTimeout) clearTimeout(saveTimeout); saveTimeout = setTimeout(() => { saveIndicatorEl.classList.remove('visible'); }, 2000); }
...
// --- INITIALIZATION ---
// ... (correct)
function initializeGame() { const savedData = localStorage.getItem('idleRPGSaveData'); if (savedData) { const loadedState = JSON.parse(savedData); let baseState = getDefaultGameState(); gameState = {...baseState, ...loadedState}; gameState.upgrades = {...baseState.upgrades, ...loadedState.upgrades}; gameState.equipment = {...baseState.equipment, ...loadedState.equipment}; if (gameState.level) { gameState.maxLevel = gameState.level; gameState.currentFightingLevel = gameState.level; delete gameState.level; } logMessage("Saved game loaded!"); } else { gameState = getDefaultGameState(); logMessage("Welcome! Your progress will be saved automatically."); } setupTabs(); generateMonster(); recalculateStats(); updateUI(); monsterImageEl.onclick = clickMonster; setInterval(autoSave, 30000); window.addEventListener('beforeunload', autoSave); }

// --- RAID FEATURE LOGIC (WITH THE FIX) ---
...
function setupRaidSocket() {
    ...
    socket.on('connect_error', (err) => { console.error("Connection Error:", err); setTimeout(() => { if(!socket.connected) { logMessage("Raid server offline."); const raidPanel = document.getElementById('raid-panel'); if(raidPanel) raidPanel.innerHTML = "<h2>Raid Server Offline</h2>"; } }, 2000); });
}
function updateRaidUI(raidState) {
    ...
    // --- FIX 2: Enable the button once we get the first update ---
...
function attackRaidBoss() {
    if (socket && socket.connected) {
        console.log(`CLIENT: Attacking with ${playerStats.totalClickDamage} damage.`);
