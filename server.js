...
io.on('connection', (socket) => {
    ...
    socket.on('updatePlayerStats', (playerData) => {
        // Find the player and update their DPS
    ...
    if (Object.keys(raidState.players).length > 0) {
        ...
        if (raidState.currentHp > 0) {
            ...
            if (raidState.currentHp <= 0) {
                ...
                io.to('raid_room').emit('raidOver', { message: "The World-Eater Ogre has been defeated! Rewards distributed!" });
        ...
        // Optionally reset the boss if everyone leaves
        // resetRaidBoss(); 
    }
}, 1000);
