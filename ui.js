 ...
 * @param {object} gameState - Pass the entire gameState to check for unlocks.
...
function createHuntCardHTML(hunt, index, isActive, gameState, progress = 0) {
    ...
    if (isActive) {
        ...
        const isTravelUnlocked = gameState.unlockedFeatures.huntTravel;
        const travelButtonDisabled = isComplete || !isTravelUnlocked;
        const travelButtonClass = !isTravelUnlocked ? 'hunt-travel-locked' : '';
        ...
        actionButtonHTML = `
            <div class="hunt-actions">
                ...
                <div class="hunt-action-buttons">
                    <button id="travel-to-hunt-btn" class="${travelButtonClass}" ${travelButtonDisabled ? 'disabled' : ''}>Travel</button>
...
/**
 * Shows the modal for selecting a monster to travel to for a hunt.
 * @param {DOMElements} elements - The main DOM elements object.
 * @param {Array<object>} travelOptions - The travel options generated by getTravelOptionsForHunt.
 * @param {number} playerMaxLevel - The highest level the player has reached.
 * @param {function(number): void} travelCallback - Function to call when a travel button is clicked.
 */
export function showHuntTravelModal(elements, travelOptions, playerMaxLevel, travelCallback) {
    elements.modalTitleEl.textContent = 'Travel to Hunt Location';
    
    const list = document.createElement('ul');
    list.className = 'wiki-travel-options-list'; // Reusing wiki styles is efficient

    if (travelOptions && travelOptions.length > 0) {
        travelOptions.forEach(option => {
            const isUnlocked = playerMaxLevel >= option.level;
            const li = document.createElement('li');
            li.className = 'wiki-travel-option';
            ...
            travelBtn.disabled = !isUnlocked;
            if (isUnlocked) {
                travelBtn.addEventListener('click', () => travelCallback(option.level));
            }
            
            li.innerHTML = `
                <img src="${option.monster.image}" alt="${option.monster.name}" class="wiki-travel-monster-icon">
                <div class="wiki-travel-monster-details">
                    <span class="wiki-monster-name">${option.monster.name}</span>
                    <span class="wiki-monster-location">${option.location}</span>
                </div>
            `;
            li.appendChild(travelBtn);
            list.appendChild(li);
        ...
        list.innerHTML = '<li>No specific travel locations could be found for this hunt.</li>';
    }
...
export function updateVolumeSlidersUI(elements, volumeSettings) {
    ...
    if (masterMuteBtn) {
        masterMuteBtn.classList.toggle('muted', volumeSettings.master === 0);
        const icon = masterMuteBtn.querySelector('i');
        if (icon) icon.className = volumeSettings.master > 0 ? 'fas fa-volume-up' : 'fas fa-volume-mute';
    }
